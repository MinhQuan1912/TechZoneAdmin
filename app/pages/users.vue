   ...
   <div class="space-y-4">
      <UCard>
         <div class="flex gap-3">
            ...
            />
            <UButton color="primary" icon="i-heroicons-magnifying-glass" @click="doSearch">Tìm</UButton>
            <UButton color="neutral" variant="outline" @click="store.search = ''; store.fetchAll()">Reset</UButton>
         </div>
      </UCard>
            ...
            <template #actions-cell="{ row }">
               <div class="flex gap-2">
                  <UButton
                     ...
                     :loading="blockingId === row.original.id"
                     @click="toggleBlock(row.original)"
                  >
                     {{ row.original.isBlocked ? 'Mở khóa' : 'Khóa' }}
                  </UButton>
                  <UButton size="xs" color="neutral" variant="outline" icon="i-heroicons-heart" @click="viewWishlist(row.original)" >Xem danh sách yêu thích</UButton>
               </div>
            </template>
            <template #empty>
               <div class="text-center py-8 text-gray-400">Không có người dùng</div>
            </template>
         </UTable>
         ...
         <UCard>  
            <template #header>
               <div class="flex items-center justify-between">
                  <div>
                     <h3 class="font-semibold">Wishlist của {{ selectedUser?.name }}</h3>
            ...
            <div v-else-if="wishlistItems.length === 0" class="text-center py-8 text-gray-400">
               ...
               <p class="text-sm">Wishlist trống</p>
            </div>
            ...
            <template #footer>
               <div class="flex items-center justify-end">
                  <UButton color="neutral" @click="wishlistOpen = false">Đóng</UButton>
               </div>
            </template>
         </UCard>
         </template>
...
async function toggleBlock(user: User) {
   ...
   try {
      await store.toggleBlock(user.id)
   } finally {
