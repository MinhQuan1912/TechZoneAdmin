<template>
   <div class="max-w-4xl mx-auto space-y-6">
      ...
      <form v-else-if="productStore.current" @submit.prevent="handleSubmit" class="space-y-6">
         <UCard>
            <template #header>
         ...
         <UCard>
            ...
            <input ref="newImageFileInput" type="file" accept="image/*" multiple class="hidden"
               ...
               <div class="grid grid-cols-4 md:grid-cols-6 gap-2">
                  <div v-for="(img, idx) in newImagePreviews" :key="idx"
                     ...
                     <button type="button"
                        ...
                        @click.stop="removeNewImage(idx)">×</button>
                  </div>
               </div>
            </div>
         </UCard>
                  ...
                  <div v-if="expanded.includes(variant.id)" class="px-4 py-4 space-y-4">
                     <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
                        ...
                        <UFormField label="Giá gốc (đ) *">
                           <UInput v-model="variantForms[variant.id].originalPrice" type="number" :min="0"
                              class="w-full" />
                        </UFormField>
                        <UFormField label="Giá bán (đ) *">
                           <div>
                              <UInput v-model="variantForms[variant.id].salePrice" type="number" :min="0"
                                 class="w-full" />
                     ...
                     <div class="border-t border-gray-100 pt-4">
                        ...
                        <div class="flex items-start gap-4">
                           ...
                           <div class="text-xs text-gray-400 space-y-1">
                              <p class="text-green-600" v-if="variant.imageUrl">Đã có ảnh màu</p>
                              <p class="text-orange-500" v-else>Chưa có ảnh</p>
                           </div>
                        </div>
                        <input :ref="el => { if (el) variantImageInputs[variant.id] = el as HTMLInputElement }"
                           type="file" accept="image/*" class="hidden"
                           @change="e => handleVariantImageChange(e, variant.id)" />
                     </div>
                     <div class="flex justify-end gap-2 pt-2 border-t border-gray-100">
                        <UButton size="sm" color="primary" :loading="savingVariantId === variant.id"
                           icon="i-heroicons-check" @click="saveVariant(variant.id)">
                           Lưu biến thể
                        </UButton>
                     </div>
                  </div>
