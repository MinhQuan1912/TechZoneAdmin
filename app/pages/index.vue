   ...
   <div class="space-y-6">
      ...
      <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
         ...
         <CommonAppStatsCard icon="i-heroicons-shopping-cart" label="Tổng đơn hàng"
            :value="dashStore.revenue?.orderCount || 0" :sub="`Đã giao: ${dashStore.revenue?.deliveredOrders || 0}`"
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                     ...
                     <div class="absolute inset-0 pb-6 flex items-end gap-px">
                        <div v-for="(item, idx) in chartData" :key="idx" class="flex-1 group relative flex items-end"
                           ...
                           pointer-events-none transition-opacity">
                              <p class="font-medium">{{ formatChartLabel(item.date) }}</p>
                              <p class="text-primary-300">{{ formatCurrency(Number(item.revenue)) }}</p>
                           </div>
                           <div
         ...
         <UCard>
            <template #header>
               <h3 class="font-semibold text-gray-900">Top bán chạy</h3>
            </template>
            <div class="space-y-3">
               <div v-for="(p, idx) in dashStore.revenue?.topProducts || []" :key="p.id"
                  ...
                  <div class="flex-1 min-w-0">
                     ...
                     <p class="text-xs text-gray-500">Đã bán: <span class="font-semibold text-primary-600">{{
                           p.totalSold
                           }}</span></p>
                  </div>
               </div>
      ...
      <UCard>
         <template #header>
            <div class="flex items-center justify-between">
               <h3 class="font-semibold text-gray-900">Đơn đã giao gần đây</h3>
         ...
         <UTable :data="recentDeliveredOrders" :columns="orderColumns">
            ...
            <template #empty>
               <div class="text-center py-6 text-gray-400 text-sm">Chưa có đơn đã giao</div>
            </template>
         </UTable>
      </UCard>
