<template>
   <div class="space-y-4">
      <div class="flex items-center justify-between">
         <UButton icon="i-heroicons-plus" color="primary" class="hover:cursor-pointer" @click="openCreate">
            Thêm danh mục
         </UButton>
      </div>
      ...
      <UCard>
         <UTable :data="store.items" :columns="columns" :loading="store.loading">
            <template #name-cell="{ row }">
               <div class="flex items-center gap-3">
                  <div
                     ...
                     <img v-if="row.original.imageUrl" :src="row.original.imageUrl" class="w-full h-full object-cover"
                        :alt="row.original.name" />
                     <UIcon v-else name="i-heroicons-photo" class="text-gray-300 w-5 h-5" />
                  </div>
                  <div>
                     <p class="font-medium">{{ row.original.name }}</p>
                     <p class="text-xs text-gray-400">{{ row.original.slug }}</p>
                  </div>
               </div>
            </template>
            <template #products-cell="{ row }">
               <UBadge :color="(row.original._count?.products || 0) > 0 ? 'primary' : 'neutral'" variant="soft">
            ...
            <template #empty>
               <div class="text-center py-8 text-gray-400">Chưa có danh mục nào</div>
            </template>
         </UTable>
      </UCard>
      ...
      <UModal v-model:open="formOpen">
         <template #content>
            <UCard>
               <template #header>
                  <h3 class="font-semibold">{{ editingCategory ? 'Sửa danh mục' : 'Thêm danh mục' }}</h3>
               </template>

               <div class="space-y-4">
                  <UFormField label="Tên danh mục *">
                     <UInput v-model="formName" placeholder="Điện thoại, Laptop..." class="w-full" />
                  </UFormField>

                  <UFormField label="Ảnh danh mục">

                     <div v-if="imagePreview || (editingCategory?.imageUrl && !removeExistingImage)"
                        class="relative w-full h-36 rounded-xl overflow-hidden mb-2 border border-gray-200 bg-gray-50">
                        <img :src="imagePreview || editingCategory?.imageUrl || ''"
                           class="w-full h-full object-cover" />
                        <UButton size="xs" color="error" variant="solid" icon="i-heroicons-x-mark"
                     ...
                     <div v-else
                        ...
                        <UIcon name="i-heroicons-cloud-arrow-up" class="w-8 h-8 text-gray-300 mx-auto mb-2" />
                        <p class="text-sm text-gray-400">Click để chọn ảnh</p>
                        <p class="text-xs text-gray-300 mt-1">PNG, JPG tối đa 5MB</p>
                     </div>
         ...
         <template #content>
            <UCard>
               <template #header>
                  <div class="flex items-center gap-3">
                     <div class="w-10 h-10 rounded-full bg-red-100 flex items-center justify-center">
                        <UIcon name="i-heroicons-exclamation-triangle" class="text-red-600 w-5 h-5" />
                     </div>
                     <h3 class="font-semibold">Xóa danh mục</h3>
                  </div>
               </template>
               ...
               <div class="space-y-3">
                  ...
                  <div v-if="(deletingCategory?._count?.products || 0) > 0" class="space-y-2">
                     <label
                     ...
                     >
                        <input type="radio" :checked="moveProducts" class="mt-0.5" readonly />
                        <div>
               ...
               <template #footer>
                  <div class="flex justify-end gap-3">
                     <UButton color="neutral" variant="outline" @click="deleteOpen = false">Hủy</UButton>
                     <UButton
