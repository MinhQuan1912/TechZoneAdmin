   ...
   <div class="flex h-[calc(100vh-120px)] bg-white rounded-2xl border border-gray-200 shadow-sm">
      ...
      <div class="w-72 shrink-0 border-r flex flex-col">
         <div class="px-4 py-3 border-b">
            ...
            <p class="text-xs text-gray-500 mt-0.5">{{ chat.rooms.value.length }} cuộc hội thoại</p>
         </div>
         ...
         <div v-else-if="!chat.rooms.value.length"
      ...
      <div class="flex-1 flex flex-col min-h-0">
         <div v-if="!chat.activeRoom.value"
            ...
            <div>
               <p class="font-medium text-gray-600">Chọn một cuộc trò chuyện</p>
               <p class="text-sm mt-1">để bắt đầu trả lời khách hàng</p>
            </div>
         </div>
         ...
         <template v-else>
            <div class="px-5 py-3 border-b flex items-center gap-3 shrink-0">
               ...
               <div>
                  ...
                  <p class="text-xs text-gray-500">{{ chat.activeRoom.value.user?.email }}</p>
               </div>
            </div>
               ...
               <template v-else>
                  <div v-for="msg in chat.messages.value" :key="msg.id" class="flex gap-2"
                     ...
                     <div :class="msg.sender?.role === 'ADMIN' ? 'items-end' : 'items-start'"
                        ...
                        <p class="text-[10px] text-gray-400 mt-1 px-1">{{ formatTime(msg.createdAt) }}</p>
                     </div>
                  </div>
